...
namespace ZPG {
    ...
    public:
        ...
        */
        ...
        template<typename T> requires std::is_base_of_v<Texture, T>
        const ref<T>& GetTexture(const std::string& name) const;
    ...
    const ref<T>& ResourceManager::GetTexture(const std::string& name) const
    {
        const ref<Texture>& texture = m_TextureLib.GetTexture(name);

        return (ref<T>&)texture;
    }
