...
namespace ColtM4CarbineScene {

ColtM4CarbineScene::ColtM4CarbineScene() : m_Controller(GetCamera()), m_LocalRes() {}

void ColtM4CarbineScene::OnLazyAttach() {
    m_LocalRes.LoadModel("Colt", "./assets/models/colt_m4a1_carbine/scene.gltf");
    ...
    for (int i = -grid_size; i < grid_size; i++) {
        for (int j = -grid_size; j < grid_size; j++) {
            ...
            GetEntityManager().AddEntity(
                new Entity(m_LocalRes.GetModel("Colt"), transform));
        }
    }
...
void ColtM4CarbineScene::OnUpdate(Timestep &ts) {
    Scene::OnUpdate(ts);
    m_Controller.OnUpdate(ts);
}

void ColtM4CarbineScene::OnEvent(Event &event) {
    Scene::OnEvent(event);
    m_Controller.OnEvent(event);
}

void ColtM4CarbineScene::OnImGuiRender() {
    Scene::OnImGuiRender();
