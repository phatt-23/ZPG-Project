...
namespace CV8
{
        ...
        : m_TransparentEntities()
    {
        ...
        auto* flashlightCameraController = new FlashlightCameraController(spotlight, GetCamera());
    ...
    void ForestScene::OnLazyAttach()
    {
        ...
        {
            ffMesh->GetMaterial()->SetMetallic(0);
            ffMesh->GetMaterial()->SetEmissive(v4(10,10,0,2));
        }

        m_LocalRes.GetModel("Bush")->GetMeshes().front()->GetMaterial()->SetMetallic(0);
        m_LocalRes.GetModel("Bush2")->GetMeshes().front()->GetMaterial()->SetMetallic(0);
        ...
        for (int i = 0; i < 50; i++)
        {
            ...
            m_TransparentEntities.AddEntity(
                new Entity(
                    m_LocalRes.GetModel(i % 2 == 0 ? "Bush" : "Bush2"),
                    transform));
        }
