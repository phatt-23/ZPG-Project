...
// Transpose to undo the undoing of the rotation, thus rotating it back to the previous rotatoin.
// Transpose doesn't effect scaling. The inverse-scale is desired. Without the inverse, when streching a triangle
// the gets progressively stretched with it. We wan't to unstrech it - we need inverse scale.
// We get a matrix that reverses the scaling but preserves the rotation of the normal vector.
